...
export const getUserInfo = async ({ userId }: getUserInfoProps) => {
  try {
    ...
    if (user.documents.length === 0) {
      console.error("No user found for the provided userId");
      return null;
    }
...
export const exchangePublicToken = async ({
  ...
  try {
    ...
    // Create a bank account using the user ID, bank ID, access token, funding source URL, and sharable ID.
...
export const getBanks = async ({ userId }: getBanksProps) => {
  try {
    ...
    console.log("Error while fetching the multiple Banks", error);
  }
};
