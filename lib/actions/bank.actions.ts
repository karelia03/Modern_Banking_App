...
export const getAccount = async ({ appwriteItemId }: getAccountProps) => {
  try {
    ...
    if (!bank) throw new Error("Bank data not found");
    ...
    const accountData = accountsResponse.data.accounts[0];
    if (!accountData) throw new Error("Account data not found from Plaid");
    ...
    if (!institution) throw new Error("Institution not found from Plaid");
